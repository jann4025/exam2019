    ...
    for (let x = 0; x < values.length; x++) {
      if (values[x] == 10) {
        let card = {
          Value: values[x],
          Suit: suits[i],
          Image: `https://deckofcardsapi.com/static/img/0${suits[i]
            .charAt(0)
            .toUpperCase()}.png`,
        ...
        let card = {
          Value: values[x],
          Suit: suits[i],
          Image: `https://deckofcardsapi.com/static/img/${values[x] +
            suits[i].charAt(0).toUpperCase()}.png`,
          ID: values[x] + suits[i].charAt(0).toUpperCase()
        };
        deck.push(card);
      }
...
function gameoverScreen() {
  ...
  if (playerSum > 21) {
    ...
    document.querySelector(
      '.gameoverScreen p'
    ).innerHTML = `Da dealeren fik ${dealerSum -
